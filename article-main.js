(function renderArticlePage() {
  ...
  heroNode.innerHTML = [
    ...
    '  <img src="' + escapeHtml(article.image.src) + '" alt="' + escapeHtml(article.image.alt) + '" loading="lazy" onerror="this.style.display=\'none\'" />',
    "</figure>"
  ].join("\n");
  ...
  var sectionMarkup = (article.sections || [])
    .map(function (section) {
      ...
      sectionParts.push("<h2>" + escapeHtml(section.heading) + "</h2>");
      ...
      if (section.guideLinkText && section.guideLinkHref) {
        sectionParts.push(
          '<p class="guide-line">👉 Explore our <a class="guide-link" href="' +
            escapeHtml(section.guideLinkHref) +
            '"' +
            buildTargetAttributes(section.guideLinkNewTab) +
            ">" +
            escapeHtml(section.guideLinkText) +
            "</a></p>"
        );
      }
